...
angular.module('angular-components.appGrid', [
  'ngGrid',
  'angular-loading-bar',
...
]).factory('GridTranslateService', [
  '$cookieStore',
  '$translate',
  function ($cookieStore, $translate) {
    return {
      translate: function (columnDefinitions, primaryKeyMeta, locale) {
        ...
        for (var p = 0; p < columnDefinitions.length; p++) {
          if (primaryKeyMeta != undefined && columnDefinitions[p].field.toLowerCase() == primaryKeyMeta.columnName.toLowerCase()) {
  ...
  function ($translate) {
    return {
      ...
      link: function ($scope, $element) {
        $($element).html('<a class=\'' + $scope.class + '\'  title=\'' + $translate.instant($scope.titleKey) + '\'><i class=\'' + $scope.class + '\'></i></a>');
      }
    };
  }
]).directive('appGrid', [
      ...
      controller: function ($scope, $attrs, $http, $resource, $translate) {
        ...
        $scope.afterSelection = function (ngRow) {
          if (ngRow.selected == true && angular.isFunction($scope.onSelectItem)) {
            if (ngRow.entity.hasOwnProperty($scope.primaryKeyName.toLowerCase()) || ngRow.entity.hasOwnProperty($scope.primaryKeyName)) {
        ...
        $scope.getList = function (pageNo) {
          ...
          $http({
            ...
            if (content.hasOwnProperty('configuration.notfound')) {
              var configurationNotFoundedMess = $translate.instant('errors.configuration.notfound') + $scope.key;
            ...
            if ($scope.columnDefs == undefined) {
              ...
              if (content.meta != null && content.meta.properties != null && content.meta.properties.primaryKey != undefined && content.meta.properties.primaryKey != '' && content.meta.properties.primaryKey != null) {
        ...
        $scope.resizeGrid = function (gridId) {
          ...
          var footerRowHeight = gridScope.showFooter == true ? gridScope.footerRowHeight == undefined ? 55 : gridScope.footerRowHeight : 0;
        ...
        $scope.options = { plugins: [$scope.layoutPlugin] };
